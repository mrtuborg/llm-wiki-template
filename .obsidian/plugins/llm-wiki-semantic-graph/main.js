...
var SemanticGraphView = class extends import_obsidian.ItemView {
  constructor() {
    ...
    this.linkDist = 90;
    this.chargeStr = 260;
    // stored positive, applied as negative
    // live D3 selections
    this.selNodeEl = null;
  ...
  applyVisibility(adj) {
    ...
    this.selNodeEl.style("opacity", (d) => {
      ...
      if (!sel) return "1";
      return d.id === sel || neighbors.has(d.id) ? "1" : "0.08";
    }).style(
      "display",
  ...
  render() {
    ...
    const mkBtn = (text, label, active = false) => {
    ...
    requestAnimationFrame(() => {
      ...
      this.sim = simulation_default(this.nodes).force("link", linkForce).force("charge", chargeForce).force("center", center_default(W / 2, H / 2)).force("collide", collide_default(22));
      ...
      nodeEl.append("circle").attr("class", "llm-graph-node-circle").attr("r", 10).attr("fill", (d) => {
        var _a2;
        return (_a2 = NODE_COLORS[d.type]) != null ? _a2 : "var(--text-muted)";
      });
  ...
  buildSidebar(sidebar, A) {
    ...
    const mkSlider = (label, key, min, max, val, step) => {
      ...
      row.createSpan({ cls: "llm-sb-slider-val", text: String(val) });
    };
    mkSlider("Link dist", "linkDist", 30, 250, this.linkDist, 5);
    mkSlider("Repulsion", "chargeStr", 50, 600, this.chargeStr, 10);
