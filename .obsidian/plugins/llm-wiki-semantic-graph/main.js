...
var SemanticGraphView = class extends import_obsidian.ItemView {
  constructor() {
    ...
    // live D3 selections (needed for toggles after render)
  ...
  async buildGraph() {
    ...
    const files = this.app.vault.getMarkdownFiles().filter(
      (f) => f.path.startsWith("wiki/") && !EXCLUDED_PATHS.some((ex) => f.path.includes(ex))
    );
    for (const file of files) {
      const fm = (_b = (_a = this.app.metadataCache.getFileCache(file)) == null ? void 0 : _a.frontmatter) != null ? _b : {};
      const type = ((_c = fm["type"]) != null ? _c : "unknown").toLowerCase();
      const domain = (_d = fm["domain"]) != null ? _d : "";
      const title = (_e = fm["title"]) != null ? _e : file.basename;
      nodeMap.set(file.basename, { id: file.basename, title, type, domain });
  ...
  computeAnalytics() {
    ...
    const density = n > 1 ? m2 / (n * (n - 1)) : 0;
    const avgDeg = n > 0 ? [...degreeOf.values()].reduce((a2, b) => a2 + b, 0) / n : 0;
  ...
  render() {
    ...
    const mkBtn = (text, title, active = false) => {
      const b = toolbar.createEl("button", { cls: "llm-graph-btn", text });
      b.setAttribute("aria-label", title);
      if (active) b.addClass("llm-graph-btn--active");
      return b;
    };
    const resetBtn = mkBtn("\u27F3", "Reset zoom");
    const nlBtn = mkBtn("\u{1F3F7} Nodes", "Toggle node labels", true);
    const elBtn = mkBtn("\u2501 Edges", "Toggle edge labels", true);
    const arBtn = mkBtn("\u2192 Arrows", "Toggle arrows", true);
    const sbBtn = mkBtn("\u{1F4CA}", "Analytics sidebar");
    toolbar.createSpan({
      cls: "llm-graph-stats",
    ...
    defs.append("marker").attr("id", "llm-arrow").attr("viewBox", "0 -4 8 8").attr("refX", 20).attr("refY", 0).attr("markerWidth", 6).attr("markerHeight", 6).attr("orient", "auto").append("path").attr("d", "M0,-4L8,0L0,4").attr("fill", "var(--text-faint)");
    ...
    requestAnimationFrame(() => {
      ...
      this.sim = simulation_default(this.nodes).force("link", link_default(simEdges).id((d) => d.id).distance(90).strength(0.4)).force("charge", manyBody_default().strength(-260)).force("center", center_default(W / 2, H / 2)).force("collide", collide_default(22));
      ...
      const nodeEl = g.append("g").selectAll("g").data(this.nodes).join("g").attr("class", "llm-graph-node").call(drag_default().on("start", (ev, d) => {
        var _a;
        if (!ev.active) (_a = this.sim) == null ? void 0 : _a.alphaTarget(0.3).restart();
        d.fx = d.x;
        d.fy = d.y;
      }).on("drag", (ev, d) => {
        d.fx = ev.x;
        ...
        d.fx = null;
        d.fy = null;
      })).on("click", (_ev, d) => this.app.workspace.openLinkText(d.id, "", false));
      nodeEl.append("circle").attr("class", "llm-graph-node-circle").attr("r", 10).attr("fill", (d) => {
        var _a;
        return (_a = NODE_COLORS[d.type]) != null ? _a : "var(--text-muted)";
      });
      ...
      nodeEl.on("mouseenter", (_ev, d) => {
        var _a, _b;
        const deg = (_a = A.degreeOf.get(d.id)) != null ? _a : 0;
        tooltip.innerHTML = `<strong>${d.title}</strong><br><span class="llm-tp-type" style="color:${(_b = NODE_COLORS[d.type]) != null ? _b : "#888"}">${d.type}</span>` + (d.domain ? `<br><em>${d.domain}</em>` : "") + `<br><small>degree: ${deg}</small>`;
      ...
      this.focusNodeFn = (id2) => {
        ...
        nodeEl.select("circle").attr("stroke-width", (d) => d.id === id2 ? 3 : 2).attr("stroke", (d) => d.id === id2 ? "#fff" : "var(--background-primary)");
      ...
      sidebar.querySelectorAll("[data-node-id]").forEach((el) => {
        el.addEventListener("click", () => {
          ...
          (_a = this.focusNodeFn) == null ? void 0 : _a.call(this, id2);
        });
      });
      this.sim.on("tick", () => {
    ...
    nlBtn.addEventListener("click", () => {
      ...
      (_a = this.selNodeLabel) == null ? void 0 : _a.attr("display", this.showNodeLabels ? null : "none");
    });
    elBtn.addEventListener("click", () => {
      var _a;
      this.showEdgeLabels = !this.showEdgeLabels;
      elBtn.toggleClass("llm-graph-btn--active", this.showEdgeLabels);
      (_a = this.selEdgeLabel) == null ? void 0 : _a.attr("display", this.showEdgeLabels ? null : "none");
    });
    arBtn.addEventListener("click", () => {
      var _a;
      this.showArrows = !this.showArrows;
      arBtn.toggleClass("llm-graph-btn--active", this.showArrows);
      (_a = this.selEdgeLine) == null ? void 0 : _a.attr("marker-end", this.showArrows ? "url(#llm-arrow)" : null);
    });
  ...
  buildSidebar(sidebar, A) {
    ...
    for (let i = 0; i < A.layers.length; i++) {
      ...
      const name = row.createSpan({ cls: "llm-sb-layer-name", text: l.name });
    ...
    A.hubs.forEach((h, i) => {
      ...
      row.setAttribute("title", `Click to focus: ${h.id}`);
      ...
      const nameEl = row.createSpan({ cls: "llm-sb-hub-name", text: h.id.length > 22 ? h.id.slice(0, 20) + "\u2026" : h.id });
      nameEl.setAttribute("title", h.id);
      const degBar = row.createDiv({ cls: "llm-sb-hub-bar" });
      const fill = degBar.createDiv({ cls: "llm-sb-fill", style: `width:${h.deg / maxH * 100}%;background:${(_b = NODE_COLORS[h.type]) != null ? _b : "#888"}` });
    ...
    for (const d of A.domains) {
      ...
      const track = row.createDiv({ cls: "llm-sb-track" });
      const fill = track.createDiv({ cls: "llm-sb-fill llm-sb-fill--teal" });
      fill.style.width = `${d.count / maxD * 100}%`;
